...
def rotate_image(image):
    ...
    rotation_matrix = cv2.getRotationMatrix2D((cols / 2, rows / 2), rotation_angle_right, 1)
    rotated_image_right = cv2.warpAffine(image, rotation_matrix, (cols, rows))
    rotation_matrix = cv2.getRotationMatrix2D((cols / 2, rows / 2), rotation_angle_left, 1)
...
if uploaded_image is not None:
    ...
    if st.button('Klasifikasikan'):
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["Original Image", "Rotated", "Brightness Changed", "Sheared"])
        with tab1:
            st.image(image, width=224)
            predicted_class, result_confidence = classify_image(image)
            st.info(f"Hasil klasifikasi citra Asli adalah ***{predicted_class}***, nilai keyakinan ***{result_confidence:.2%}***", icon="ℹ️")
        ...
        with tab2:
            ...
            with col1:
                ...
                predicted_class, result_confidence = classify_image(rotated_image_right)
                st.info(f"Hasil klasifikasi citra Rotasi 30° Ke Kiri adalah ***{predicted_class}***, nilai keyakinan ***{result_confidence:.2%}***", icon="ℹ️")


            with col2:
                st.image(rotated_image_left, width=224)
                predicted_class, result_confidence = classify_image(rotated_image_left)
                st.info(f"Hasil klasifikasi citra Rotasi 30° Ke Kanan adalah ***{predicted_class}***, nilai keyakinan ***{result_confidence:.2%}***", icon="ℹ️")
        ...
        with tab3:
            ...
            with col1:
                ...
                predicted_class, result_confidence = classify_image(brightened_image)
                st.info(f"Hasil klasifikasi citra Kecarahan +50% adalah ***{predicted_class}***, nilai keyakinan ***{result_confidence:.2%}***", icon="ℹ️")

            with col2:
                st.image(darkened_image, width=224)
                predicted_class, result_confidence = classify_image(darkened_image)
                st.info(f"Hasil klasifikasi citra Kecarahan -50% adalah ***{predicted_class}***, nilai keyakinan ***{result_confidence:.2%}***", icon="ℹ️")

        with tab4:
            st.image(shear_image, width=224)
            predicted_class, result_confidence = classify_image(shear_image)
            st.info(f"Hasil klasifikasi citra Shear adalah ***{predicted_class}***, nilai keyakinan ***{result_confidence:.2%}***", icon="ℹ️")
